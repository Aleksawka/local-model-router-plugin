...
| Plugin может назначить Senior/Junior из Copilot-imported моделей | Подтверждено для setting + agent `model:`; native App Settings UI у plugin нет |
| Plugin читает официальный API каталога Copilot App | Нет, discovery best-effort по локальным JSON и экспорту picker |
...
Именно поэтому версия `0.2.0` по-прежнему запускается в audit mode. Назначение Senior/Junior из импортированных моделей ортогонально гипотезе hook. Официальный тип `toolArgs` — `unknown`; CLI changelog подтверждает `task(agent_type=...)`, но App может отдать `Agent` + `tool_input` + `subagent_type`. Жёстко предполагать одну форму до наблюдения нельзя.

## Почему это не замена Copilot Auto

Есть четыре разных точки решения:

1. Пользователь назначает, какая Copilot-imported модель Senior, а какая Junior.
2. Copilot `Auto` выбирает модель родительского хода до отправки запроса провайдеру.
3. Senior внутри своего ответа решает, создавать ли `task`.
4. Plugin hook может изменить профиль только уже созданного `task`.
...
Версия после `0.2.0` может добавить отдельный daemon или MCP/harness для:
